...
/* AWH - We need to increase the size of the opcode buffer to be big
  ...
  we can't just increase the size of the opcode buffer without keeping
  OPC_MAX_SIZE at its original size because otherwise larger TBs will be 
  generated by the TCG to take up all the extra buffer space, leaving us 
  with huge TBs that, once again, don't have space for the IRs we need to
  add. I discovered this problem when I started to get blocks with 6000 or 
  7000 IRs in them. 

  To fix this, we "squeeze" the capacity of the buffer to its original,
  smaller size by lying to the gen_intermediate_code() logic.  We say that 
  MAX_OP_PER_INSTR is much bigger than it actually is. 
  gen_intermediate_code() stops adding new opcodes when it gets within 
  MAX_OP_PER_INSTR of the end of the buffer.
  ...
  |              gen_opc_buf                       |
  +------------------------------------------------+
  <- OPC_MAX_SIZE: 432 -><- MAX_OP_PER_INSTR: 208 ->

  NEW: Increase MAX_OP_PER_INSTR and OPC_BUF_SIZE by the same amount to
  set aside excess buffer for taint IR and log CALL opcodes.
...
#define CODE_GEN_AVG_BLOCK_SIZE 128*4 //128 !!!taintcheck!!! AWH
#else
